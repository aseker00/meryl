...
ft_model = None
model_path = Path("model/ft/cc.he.300.bin")


def load_embedding_weight_matrix(home_path: Path, file_path: Path, words: list, device) -> nn.Embedding:
    path = home_path.joinpath(file_path)
    if path.exists():
        print(f"Loading FastText vectors from {path}")
        indices, vectors = _load_vec_file(path)
        ...
        if len(new_words) > 0:
            new_word_vectors = get_word_vectors(home_path, new_words)
            word_vectors.extend(new_word_vectors)
    else:
        word_vectors = get_word_vectors(home_path, words)
        if words[0] == '<PAD>':
            word_vectors[0] = [0] * len(word_vectors[0])
        _save_vec_file(path, words, word_vectors)
...
def get_word_vectors(home_path: Path, words: list) -> list:
    global ft_model
    if not ft_model:
        path = home_path.joinpath(model_path)
        print(f"Loading FastText model from {path}")
        ft_model = fasttext.load_model(f'{path}')
