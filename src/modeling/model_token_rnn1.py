...
class TagRNN(nn.Module):
    ...
    def forward(self, token_seq: torch.Tensor, seq_lengths: torch.Tensor, gold_morph_seq: torch.Tensor):
        ...
        preds = torch.stack([pred_pref_forms, pred_pref_tags, pred_pref_feats, pred_host_forms, pred_host_tags, pred_host_feats, pred_suff_forms, pred_suff_tags, pred_suff_feats], dim=0)
        ...
        losses = torch.stack([pref_form_loss, pref_tag_loss, pref_feats_loss, host_form_loss, host_tag_loss, host_feats_loss, suff_form_loss, suff_tag_loss, suff_feats_loss])
