...
from src.modeling.model_token_rnn1 import *
...
def get_model(hidden_size: int, num_layers: int, emb_dropout: float, hidden_dropout: float, rnn_dropout: float, class_dropout: float) -> TagRNN:
    token_emb = model_ft.load_embedding_weight_matrix(home_path, ft_token_vec_file_path, tb_vocab.tokens, device)
    model = TagRNN(token_emb, emb_dropout, hidden_size, num_layers, hidden_dropout, rnn_dropout, class_dropout, tb_vocab)
...
def tensors_to_sentences(token_seq: torch.Tensor, morph_seq: torch.Tensor, model: TagRNN) -> list:
...
def process_step(batch: tuple, model: TagRNN, optimizer: ModelOptimizer):
...
def process(epoch: int, print_every: int, phase: str, data: DataLoader, model: TagRNN, optimizer=None):
    total_losses, print_losses = [0] * 9, [0] * 9
    ...
    for j, batch in enumerate(data):
        ...
        if step % print_every == 0:
            ...
            print(f'epoch {epoch} {phase} step {step} pref tag loss {print_losses[1] / print_every}')
            print(f'epoch {epoch} {phase} step {step} pref feats loss {print_losses[2] / print_every}')
            print(f'epoch {epoch} {phase} step {step} host form loss {print_losses[3] / print_every}')
            print(f'epoch {epoch} {phase} step {step} host tag loss {print_losses[4] / print_every}')
            print(f'epoch {epoch} {phase} step {step} host feats loss {print_losses[5] / print_every}')
            print(f'epoch {epoch} {phase} step {step} suff form loss {print_losses[6] / print_every}')
            print(f'epoch {epoch} {phase} step {step} suff tag loss {print_losses[7] / print_every}')
            print(f'epoch {epoch} {phase} step {step} suff feats loss {print_losses[8] / print_every}')
    ...
    print(f'epoch {epoch} {phase} total pref tag loss {total_losses[1] / print_every}')
    print(f'epoch {epoch} {phase} total pref feats loss {total_losses[2] / print_every}')
    print(f'epoch {epoch} {phase} total host form loss {total_losses[3] / print_every}')
    print(f'epoch {epoch} {phase} total host tag loss {total_losses[4] / print_every}')
    print(f'epoch {epoch} {phase} total host feats loss {total_losses[5] / print_every}')
    print(f'epoch {epoch} {phase} total suff form loss {total_losses[6] / print_every}')
    print(f'epoch {epoch} {phase} total suff tag loss {total_losses[7] / print_every}')
    print(f'epoch {epoch} {phase} total suff feats loss {total_losses[8] / print_every}')
...
rnn = get_model(hidden_size=64, num_layers=1, emb_dropout=0.3, hidden_dropout=0.1, rnn_dropout=0.0, class_dropout=0.0)
print(rnn)
