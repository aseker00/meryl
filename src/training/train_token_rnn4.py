...
from src.modeling.model_token_rnn4 import *
...
def get_model(hidden_size: int, num_layers: int, emb_dropout: float, hidden_dropout: float, rnn_dropout: float, class_dropout: float) -> TagRNN4:
    token_emb = model_ft.load_embedding_weight_matrix(home_path, ft_token_vec_file_path, tb_vocab.tokens, device)
    model = TagRNN4(token_emb, emb_dropout, hidden_size, num_layers, hidden_dropout, rnn_dropout, class_dropout, tb_vocab)
...
def tensors_to_sentences(token_seq: torch.Tensor, morph_seq: torch.Tensor, model: TagRNN4) -> list:
...
def process_step(batch: tuple, model: TagRNN4, optimizer: ModelOptimizer):
...
def process(epoch: int, print_every: int, phase: str, data: DataLoader, model: TagRNN4, optimizer=None):
...
rnn = get_model(hidden_size=32, num_layers=1, emb_dropout=0.0, hidden_dropout=0.1, rnn_dropout=0.0, class_dropout=0.0)
print(rnn)
