...
from src.modeling.model_token_rnn5 import *
...
def get_model(hidden_size: int, num_layers: int, emb_dropout: float, hidden_dropout: float, rnn_dropout: float, class_dropout: float) -> TagRNN5:
    token_emb = model_ft.load_embedding_weight_matrix(home_path, ft_token_vec_file_path, tb_vocab.tokens, device)
    model = TagRNN5(token_emb, emb_dropout, hidden_size, num_layers, hidden_dropout, rnn_dropout, class_dropout, tb_vocab)
...
def tensors_to_sentences(token_seq: torch.Tensor, morph_seq: torch.Tensor, model: TagRNN5) -> list:
...
def process_step(batch: tuple, model: TagRNN5, optimizer: ModelOptimizer):
...
def process(epoch: int, print_every: int, phase: str, data: DataLoader, model: TagRNN5, optimizer=None):
    ...
    for j, batch in enumerate(data):
        ...
        if step % print_every == 0:
            ...
            print(f'epoch {epoch} {phase} step {step} host feats loss {print_losses[0] / print_every}')
    ...
    print(f'epoch {epoch} {phase} total host feats loss {total_losses[0] / print_every}')
